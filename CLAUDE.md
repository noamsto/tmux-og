...
| `tmux-reap-pane` | `pane-exited`/`pane-died` window hooks with `#{q:hook_pane}` | Deletes exactly that pane's `panes`/`screen`/`interrupt`/`tasks`/`issues`/`watchers` files via `claude_reap_pane`; never iterates a directory (`CLAUDE_STATUS_DIR` is shared by every tmux server and pane ids are per-server `%N` counters). Ownership guard reads the state file's `session=` field and skips when this server has no such session (residuals: same-named sessions on two servers; `rename-session` fails closed to the backstop). `names/`/`live/` excluded (monotonic ids, prune owns them). |
