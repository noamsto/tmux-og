...
- **Zoom crosses the bridge as a ctl verb, not a local `resize-pane -Z`.** A local zoom does grow the renderer pane, and it sticks — but it does not touch the remote pane, so the remote program keeps rendering at its old size and the rows it gained are dead space. Measured on a 150x40 two-pane mirror: local zoom gives `dst 150x39` against `src 150x19`; the `zoom` verb gives `150x39` on both. `prefix + z` therefore sends the verb, and the mirror learns the state from `#{window_layout}` and the zoom flag (the `Z` in `window_raw_flags`) carried in the `%layout-change` notification itself (`<window-id> <window_layout> <window_visible_layout> <window_raw_flags>`, `control-notify.c:79`), not a fresh `readLayout` round-trip: `reconcileLayoutFrom` gates the notification, and a line that reports nothing the mirror doesn't already reflect — a duplicate, an echo of the daemon's own verb, the trailing line of a push/pop-zoom bracket — returns with zero remote round-trips, while a geometry-only reshape (same panes, same floats, zoom flag off, no mirrored float held) is applied straight from the notification's layout string with none either. A zoom transition, any pane-set or float change, and a reshape landing on a window still holding a mirrored float all still fall through to `reconcileLayout`'s read-first entry, because `window_push_zoom`/`window_pop_zoom` bracket structural commands with transient unzoomed lines and a stale structural line applied verbatim would do renderer surgery on a pane the remote has already closed. The live dispatch path is left uncoalesced on purpose — the trailing re-read, not a coalesce pass, is what corrects a notification the remote has already moved past (#570). **tmux exposes zoom only as a toggle, so reconcile asserts the remote flag on the mirror with an idempotent `if -F` after `applyLayout` (which may unzoom via `select-layout`) and before FrameResize/reseed — never a bare toggle, and never a per-reconcile `display-message` of local state.** Zoom-on targets the tiled pane rendering `remoteActive` (skipped when that pane is a float, #517); unzoom targets the window. Last successfully asserted flag lives in `mirrorWindow.appliedZoom` for dedup against `readLayout`'s remote flag. The ctl `zoom` verb still owns the remote side. `#{window_layout}` stays the **unzoomed** geometry deliberately: `#{window_visible_layout}` reports a zoomed window as single-pane, and reconcile would read the hidden panes as closed and kill their renderers on every toggle. A zoom made by any other client on the remote follows too: tmux emits `%layout-change` for one, even though `#{window_layout}` itself is unchanged by it — which is why reconcile's trailing re-read compares the zoom flag alongside the layout string (#413).
- **A remote float is mirrored as a local float, and a `select-layout` behind one has to drop it first.** `ParseLayout` prunes floats out of the tiled tree into `Layout.Floats`, so a float's *presence* never changes `L.Raw` — but tmux counts a floating pane against the layout's cell arithmetic, and there is no float-tolerant `select-layout`: the tiled-only string reports `have N panes but need M`, and tmux's own emitted float-bearing string does not even parse back in. So reconcile diffs `L.Floats` separately (`reconcileFloats`), and when the tiled tree genuinely moved behind an open float it first tries a **geometry short-circuit** — after the window fit, re-read the mirror's own `#{window_layout}` and compare cells pairwise against the remote's; a match means the fit alone already reproduced the remote's geometry and the `select-layout` is skipped, touching no float. The short-circuit runs before any float bookkeeping is consulted, so it also covers a float the daemon did not create — keying it on `w.localFloats` skipped it entirely for one of those and went straight to a `select-layout` that could only fail, freezing the mirror for as long as the float stayed open (#535). Only on a miss does it drop the mirrored floats, shape, and let the same pass re-add them. Two geometry spaces are in play and mixing them is the easy bug: a float's **layout cell is the inner box** and equals its usable pane size (so it feeds renderer dims unconverted), while `new-pane -x/-y/-X/-Y`, `resize-pane -x/-y`, `move-pane -X/-Y` and the `@float_geom` stamp all speak the **outer box** — inset 1 per side for every border style, 0 only for `-B none`. A float this daemon did not create (`prefix + b`/`k`/`i` are unguarded float binds) is never killed — it is not ours to reap. Since #535 that costs nothing while the tiled tree is unchanged, which is the common case; only a *genuine reshape* arriving behind one still fails, and there the resize/reseed broadcast is skipped so the mirror keeps its last-good screen rather than painting the remote's dims into panes that never took them. `retryFailedShapes` (main loop, beside `healLostWindows`) then re-drives reconcile once that float is gone: `reconcileLayoutFrom` is the entry a `%layout-change` reaches first, but a window still holding a mirrored float always falls through it to `reconcileLayout`'s read-first entry, and reconcile still runs only on remote events — a `%layout-change`, a coalesced batch, or a reattach — so closing a *local* float triggers nothing, and the stale shape otherwise survived until the remote next moved that window. Gated on a local float check, since a retry while the float is still open can only fail again and costs a round-trip. tmux is the limit here, not tmux-og: it has no float-tolerant `select-layout` at all — even its own `#{window_layout}` does not parse back in — while `resize-pane`, `split-window` and `kill-pane` are unaffected. Pending upstream rather than permanent: tmux/tmux#5577 was closed in favour of tmux/tmux#5390, which adds a JSON layout format (`"V":2`, floats carried as leaves with a `"z"` z-index) whose emitted string is meant to parse back in, opt-in for control clients via `refresh-client -f new-layouts`. If it lands, reconcile can feed the remote's string to the local `select-layout` verbatim and the short-circuit/drop-and-re-add path above goes away; note it also changes the v1 dump to omit floats, which would make our trailing-`<…>` parse branch dead code.
...
- **`@pane_keys_raw`** — pane-scoped, stamped only on the `prefix + s` `^o` float. A floating pane, unlike the popup the local picker runs in, sits under the root key table, so `C-hjkl` and `M-l` reach the bind rather than the remote picker whose own keymap they are. `tmux-smart-nav` takes the flag as its last argument and sends the key on instead of navigating; `M-l` gates on it directly in the bind.
