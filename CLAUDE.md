...
| `tmux-client-theme` | `client-light-theme[40]` / `client-dark-theme[40]` hooks (a brace-block body that first stamps `@og_client_theme_want` synchronously, then `run-shell -b`) | Follows the terminal's own reported theme (DSR 997, sent at attach and on every terminal theme change, #663). tmux fires the hook on **every** report, not only a change, so the no-op guard lives here: exits when `theme-state.json` and `@catppuccin_flavor` already match the want. Serialized by a per-user mkdir lock; each pass re-reads the newest `@og_client_theme_want`, so the last *report* wins rather than the last job to finish, and a want that did not converge is logged once and not retried. Applies through `theme-toggle apply` when on PATH; otherwise writes the state file in theme-toggle's schema, clears `@thm_*`, sets the flavor and re-sources `~/.config/tmux/tmux.conf`, replaying catppuccin + `tmux-apply-theme-colors` + `og-remote-theme` directly when that file is missing, rejected (#407), or leaves `@thm_bg` empty. The hook body must be a `{ … ; … }` block — a quoted value with an embedded `\;` is re-split into too many `set-hook` arguments and silently not set. |
...
- **Theme support**: Scripts detect light/dark from `$XDG_STATE_HOME/theme-state.json` and use Catppuccin Latte/Mocha colors accordingly. Two signals drive it, and the most recent wins: an external `theme-toggle` run, and a terminal theme report (tmux's `client-*-theme` hooks → `tmux-client-theme`). Either one converges the state file, `@catppuccin_flavor` and `@thm_*` — they must agree, since the shell/Go readers (`lib-claude.sh`, `themestate.Detect`) read the file while tmux renders the flavor. The flavor is server-global, so with several attached clients the last one to attach or change theme holds. Control-mode clients (the remote bridge) have no tty and never fire the hooks, so a bridge attach cannot flip the theme; a terminal that never answers DSR 996 never fires either. A terminal pinned opposite the desktop would revert the desktop on every attach — `set -g @og_follow_client_theme off` disables the handler at runtime. Design: `docs/superpowers/specs/2026-09-16-client-theme-hooks-design.md`.
