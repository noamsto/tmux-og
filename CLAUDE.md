...
| `tmux-shell-prompt` | `pane-shell-prompt` hook (OSC 133;A) with `#{q:hook_pane} #{qs:pane_current_command} #{qs:session_name}` | Clears an exited agent's state from the shell's own prompt mark (#646): `claude_clear_agent_state` removes `panes`/`screen`/`interrupt` for that pane and empties `@claude_status`/`@agent_screen` (the bridge shipper's source). The discriminator is the same `@AGENT_COMMANDS@` manifest the sweep arms from: a nested prompt inside a still-running agent keeps the agent as `pane_current_command` and is skipped; `session_name` (never `session_id`, whose `$N` re-expands in `run-shell`) carries the ownership guard matching the file's `session=`. |
