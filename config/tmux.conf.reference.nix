...
# work through; its four boundaries are the ones the freeze was verified at.
{
  ...
  statusText = ''
    ...
    # CPU load.
    set -g status-format[0] "#(echo; ${script.tmux-update-icons}/bin/tmux-update-icons #{qs:session_name} '#{@resume_claude}' '#{start_time}' '#{@resume_carousel}')#(${picker-statusline-bin} --session #{qs:session_name} --thm-bg '#{@thm_bg}' --thm-red '#{@thm_red}' --thm-mauve '#{@thm_mauve}' --thm-blue '#{@thm_blue}' --thm-text '#{@thm_fg}' --thm-subtext0 '#{@thm_subtext_0}' --thm-overlay1 '#{@thm_overlay_1}' --thm-peach '#{@thm_peach}' --thm-green '#{@thm_green}' --icon-session '#{@icon_session}' --icon-branch '#{@icon_branch}' --icon-dir '#{@icon_dir}' --icon-remote '#{@icon_remote}' --icon-linear '${enrichIconsDoubled.linear}' --icon-github '${enrichIconsDoubled.github}'${lib.optionalString agentUsageEnable " --icon-usage-claude '${processIcons.claude or "🧠"}' --icon-usage-codex '${processIcons.codex or "🤖"}' --icon-usage-cursor '${processIcons."cursor-agent" or "🧊"}' --agent-usage-monthly-threshold '${toString agentUsageMonthlyThreshold}'"})"
  ...
  hooksText = ''
    ...
    set-hook -g 'client-light-theme[40]' { set -g @og_client_theme_want light ; run-shell -b "${script.tmux-client-theme}/bin/tmux-client-theme" }
    set-hook -g 'client-dark-theme[40]' { set -g @og_client_theme_want dark ; run-shell -b "${script.tmux-client-theme}/bin/tmux-client-theme" }
