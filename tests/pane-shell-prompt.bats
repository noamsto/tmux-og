...
# bridge options, the exact set claude_clear_agent_state clears.
seed_shell_state() {
	local id sess
	id="$(bare_id "$1")"
	sess="$2"
...
@test "nested prompt inside a live agent: not cleared" {
	cat >"$TEST_HOME/pi-fake" <<'EOF'
#!/usr/bin/env bash
exec -a pi bash -c 'printf "\033]133;A\033\\"; sleep 3'
EOF
	chmod +x "$TEST_HOME/pi-fake"
	...
	# The fake agent's argv[0] is `pi` (in the manifest) while it runs and emits
	# a nested A — the handler must read it as a live agent and skip.
	t send-keys -t "$id" "$TEST_HOME/pi-fake" Enter
	sleep 1
	three_present "$id"
