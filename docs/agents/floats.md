...
- **Popups are modal floats (#725).** Upstream `34cd5da4` removes popups;
  ...
  **modal floating pane** in the target window instead — `new-pane -O` under
  ...
  built around that. **The chrome rule:** a pane with `pane_modal_flag=1` is
  transient chrome, never window content.
  ```
  ...
  every `display-popup` call or its pane lingers dead inside a mirror window.
