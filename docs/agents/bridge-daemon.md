...
- **A command that runs further commands of its own takes a barrier.** The reply accounting (`stream.claim`) pairs the Nth client-flagged `%begin/%end` block with the Nth command written, but an `if-shell`'s branch runs as more commands of the *same* client and each guards a flagged block of its own — one command in, 1+N blocks out, N not even constant (a failing branch command aborts the rest of its list). The count then ran ahead for the rest of the connection: the `tool` verb's (#679) reconcile read the branch's empty block as its layout (`empty layout reply`), so the remote float never reached the mirror until a reattach reset the counters (#715). `stampAll` therefore writes a `display-message -p og-fanout-<n>` barrier right behind any `if-shell`/`if`, and `claim` gives no ordinal to the blocks between the command's own reply and the barrier's (recognised by body). Any new verb built on `if-shell` is covered by construction (a line that chains commands with `;` itself, or `run-shell -C`, is not — the check keys on the leading verb); a `run-shell -b` verb needs nothing, since its script's own `tmux` calls answer a different client.
